...
static void  decrypt_cw_ex(int32_t tag, int32_t a, int32_t b, int32_t c, uint8_t *data)
{
	...
	if(tag != 0x120 && tag != 0x100 && tag != 0x10A && tag != 0x101 && tag != 0x47 && tag != 0x92 && tag == 0xDE) {return;}
	...
	des_ecb_decrypt(data, deskey1, 16);
	des_ecb_encrypt(data, deskey2, 16);
	des_ecb_decrypt(data, deskey1, 16);
}
...
static int32_t streamguard_card_init(struct s_reader *reader, ATR* newatr)
{
	...
	rdr_log(reader, "StreamGuard cas_version %d.%d card detected.",reader->cas_version / 10, reader->cas_version % 10);
	...
	if(cas_version >= 20){
		...
		if((cta_res[cta_lr - 2] & 0xF0) != 0x60){
			rdr_log(reader, "error: init begin cmd8 failed.");
			return ERROR;
		}
		data_len = streamguard_read_data(reader,cta_res[cta_lr - 1], data, &status);
		if(data_len < 0){
			rdr_log(reader, "error: init read data failed for begin cmd8.");
			return ERROR;
		}
		...
		if(cas_version >= 30){
			des_ecb_encrypt(randkey, key1, 16);
			des_ecb_decrypt(randkey, key2, 16);
			des_ecb_encrypt(randkey, key1, 16);
...
static int32_t streamguard_do_ecm(struct s_reader *reader, const ECM_REQUEST *er, struct s_ecm_answer *ea)
{
	...
	if(data_len <= 18) {return ERROR;}
	uint16_t tag=0;
	for(i = 0; i < (data_len - 1); i++)
	{
		if (cas_version >= 30 && data[i] == 0xB4 && data[i + 1] == 0x04) {tag = b2i(2, data + i + 4);}
		if (data[i] == 0x83 && data[i + 1] == 0x16)
		{
			if(cas_version <= 20 || data[i + 2] != 0 || data[i + 3] != 1) {break;}
		}
	}
	...
	if(((uint16_t)(ea->cw[0]) + (uint16_t)(ea->cw[1]) + (uint16_t)(ea->cw[2])) == (uint16_t)(ea->cw[3])
		...
		&& ((uint16_t)(ea->cw[12]) + (uint16_t)(ea->cw[13]) + (uint16_t)(ea->cw[14])) == (uint16_t)(ea->cw[15])) {return OK;}

	if((data[i + 5] & 0x10) != 0){
		uint8_t key1[8], key2[8];
		memcpy(key1, reader->des_key, 8);
		memcpy(key2, reader->des_key + 8, 8);
		des_ecb_decrypt(ea->cw, key1, sizeof(ea->cw));
		des_ecb_encrypt(ea->cw, key2, sizeof(ea->cw));
		des_ecb_decrypt(ea->cw, key1, sizeof(ea->cw));
	}
...
static int32_t streamguard_card_info(struct s_reader *reader)
{
	...
	for(i = 0; i < reader->nprov; i++){
		...
		for(j = 0; j < count; j++){
				...
				if((uint32_t)start_t == 0xFFFFFFFFLU) {start_t = subscription_t;}
				...
				if(!j){rdr_log(reader, "entitlements for provider: %d (%04X:%06X)", i, reader->caid, b2i(2, &reader->prid[i][2]));}
