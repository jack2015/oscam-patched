...
static size_t jet_encrypt(struct s_reader* reader,uint8_t ins, uint8_t *data, size_t len, uint8_t *out, size_t maxlen)
{
	...
	if((aligned_len + 7) > maxlen || (aligned_len + 7) > 256){return 0;}
	...
	if(ins == 0x15){
		...
		for(i = 0; i < (aligned_len / 8); i++){des_ecb_encrypt(buf + 8 * i, reader->jet_vendor_key + (i % 4) * 8, 8);}
...
static int32_t cw_is_valid(unsigned char *cw)
{
	int32_t i;
	for(i = 0; i < 16; i++){if(cw[i] != 0){return OK;}}
	return ERROR;
}
...
static int generate_derivekey(struct s_reader *reader, uint8_t * out, int len)
{
	...
	if(len < (int)sizeof(derivekey)){return 0;}
...
static int32_t jet_resync_vendorkey(struct s_reader *reader)
{
	...
	if(37 > twofish(cta_res + 5, cta_res[4], buf, sizeof(buf), reader->jet_vendor_key, sizeof(reader->jet_vendor_key), TWOFISH_MODE_DECRYPT)){return -1;}
	DH_Agree_Key_Gen(buf + 5, 32, x, 16, k1);
	if(reader->cas_version > 52 && !memcmp(buf + 0x25, X, 32)){return -1;}
...
static int32_t jet_card_init(struct s_reader *reader, ATR *newatr)
{
	...
	if(reader->boxkey_length != 32){memset(reader->boxkey, 0xEE, 32);}

	if(cas_version <= 52){
		for(i = 0; i < 32; i++){reader->jet_vendor_key[i] = i;}
	}else{
...
static int32_t jet_do_ecm(struct s_reader *reader, const ECM_REQUEST *er, struct s_ecm_answer *ea)
{
	...
	if(cas_version >= 5){memcpy(cmd + ecm_len + 38, reader->jet_service_key, 8);}
	...
	for(i = 0; i < (cta_res[4] / 8); i++){des_ecb_decrypt(temp + 5 + 8 * i, reader->jet_vendor_key + (i % 4) * 8, 8);}
	if(temp[9] == 0xFF){
...
static int32_t jet_card_info(struct s_reader *reader)
{
	...
	entitlements_count = (int)buf[4];
	memcpy(get_entitlements_cmd2 + 7, reader->boxkey, 32);
	get_entitlements_cmd2[39] = 0x01;
	for(i = 40; i < 48; i++){get_entitlements_cmd2[i] = cas_version < 40 ? 0x09 : 0x30;}
	...
	for(i=0; i < entitlements_count; page++){
		...
		for(k=0; k < (int)buf[4]; k++, i++){
			...
			if (!i){rdr_log(reader, "entitlements for (%04X:%06X):",  reader->caid, 0);}
